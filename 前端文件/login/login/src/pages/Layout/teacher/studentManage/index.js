...
/**************************************************************************** */
        ...
        const studentDelete =async(data)=>{
         const res =  await http.delete(`/student/${data.studentId}`)
          if (res.code===200) {
  ...
  return (
    <div>
      ...
      >
        ...
        >    
            输入学号：
            <Form.Item name="studentId" className='selectInput'>
              <Input ></Input>
            </Form.Item>
          <Form.Item>
            <Button 
          ...
          <Row><Button type="primary" onClick={showDrawer} icon={<PlusOutlined />} className='addButton'>add</Button></Row>
        ...
        {/**文章区域 */}
        <Card title={`根据筛选条件共查询到 ${Alist.count} 条结果：`} className="select_student">
        <Table 
        rowKey="id" 
        columns={columns} 
...
export default StudentManageByTea