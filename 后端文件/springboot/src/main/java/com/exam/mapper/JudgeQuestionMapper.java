...
public interface JudgeQuestionMapper {
    ...
    @Insert("insert into judge_question(subject,question,answer,level) values " +
            "(#{subject},#{question},#{answer},#{level})")
    int add(JudgeQuestion judgeQuestion);

    @Update("UPDATE exam.judge_question\n" +
            "SET subject=#{subject}, question=#{question}, answer=#{answer}\n" +
            "WHERE questionId=#{questionId};\n")
    int updateJudgeQuestion(JudgeQuestion judgeQuestion);

    @Delete("DELETE FROM exam.judge_question\n" +
            "WHERE questionId=#{questionId};\n")
