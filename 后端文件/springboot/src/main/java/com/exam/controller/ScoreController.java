...
public class ScoreController {
    ...
    public ApiResult findAllPage(@PathVariable("page") Integer page, @PathVariable("size") Integer size) {
        ...
        IPage<Score> res = scoreService.findAllPage(scorePage);
...
//    不分页
    ...
    @PostMapping("/score")
    @CrossOrigin
    public ApiResult add( Score score) {
    ...
    @DeleteMapping("/score/{examCode}/{studentId}")
    @CrossOrigin
    public ApiResult deleteScore(@PathVariable("examCode") Integer examCode,@PathVariable("studentId") Integer studentId){
        int scores = scoreService.deleteScore(examCode,studentId);
        return ApiResultHandler.buildApiResult(200,"删除成功",scores);
    }
}
